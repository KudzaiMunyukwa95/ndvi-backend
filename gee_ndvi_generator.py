    ...
    try:
        ...
        if index_type == "RADAR":
            ...
            return jsonify({
                ...
                "index": "RADAR",
                "tile_url": tile_url,
                "explanation": "Sentinel-1 RADAR Multi-Band Composite - Cloud Penetrating | Brown=Bare Soil, Green=Vegetation, Blue=Water, Yellow=Mixed",
...
def advanced_report():
    try:
        ...
        logger.info(f"[VALIDATE DEBUG] use_radar={use_radar}")
        logger.info(f"[VALIDATE DEBUG] indices keys before validation: {list(indices.keys())}")
        logger.info(f"[VALIDATE DEBUG] indices.get('data_source'): {indices.get('data_source', 'MISSING')}")
        
        validated_indices = validate_indices(indices)
        
        # DEBUG: Log what we got after validation
        logger.info(f"[VALIDATE DEBUG] validated_indices keys: {list(validated_indices.keys())}")
        logger.info(f"[VALIDATE DEBUG] validated_indices.get('data_source'): {validated_indices.get('data_source', 'MISSING')}")
